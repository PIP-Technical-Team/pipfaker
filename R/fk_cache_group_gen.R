...
fk_cache_group_gen <- function(n_quantiles = 20,
  ...
  if(!is.null(path)){
    ...
    cli::cli_abort("Make sure path exists")
  ...
  fake_svy <- fk_uniq(svy_ind_rural, n_quantiles)
  ...
  fake_svy <- fake_svy[
    ,
    welfare := wbpip::md_compute_quantiles(welfare, n_quantile = n_quantiles)
  ][
    ,
    weight  := 1/n_quantiles
  ]
